...
class Analytics:
    ...
    def get_main_stats(self):
        date_2024 = datetime.strptime("2024-01-01", "%Y-%m-%d")
        date_2023 = datetime.strptime("2023-01-01", "%Y-%m-%d")
        ...
        stats = {
            ...
            'выпущенных в 2023' : len(df[ (df['issuedate'] >= date_2023) & (df['issuedate'] <= date_2024)].index),
            'выпущенных в 2022' : len(df[ (df['issuedate'] >= date_2022) & (df['issuedate'] <= date_2023)].index),
